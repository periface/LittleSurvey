...
{
    public abstract class LittleSurveyTestBase : AbpIntegratedTestBase<LittleSurveyTestModule>
    {
        private DbConnection _hostDb;
        private Dictionary<int, DbConnection> _tenantDbs; //only used for db per tenant architecture

        protected LittleSurveyTestBase()
        {
        ...
        protected void UsingDbContext(Action<LittleSurveyDbContext> action)
        {
            UsingDbContext(AbpSession.TenantId, action);
        }

        protected Task UsingDbContextAsync(Func<LittleSurveyDbContext, Task> action)
        {
            return UsingDbContextAsync(AbpSession.TenantId, action);
        }

        protected T UsingDbContext<T>(Func<LittleSurveyDbContext, T> func)
        {
            return UsingDbContext(AbpSession.TenantId, func);
        }

        protected Task<T> UsingDbContextAsync<T>(Func<LittleSurveyDbContext, Task<T>> func)
        {
            return UsingDbContextAsync(AbpSession.TenantId, func);
        }

        protected void UsingDbContext(int? tenantId, Action<LittleSurveyDbContext> action)
        {
            using (UsingTenantId(tenantId))
            {
                using (var context = LocalIocManager.Resolve<LittleSurveyDbContext>())
                {
        ...
        protected async Task UsingDbContextAsync(int? tenantId, Func<LittleSurveyDbContext, Task> action)
        {
            using (UsingTenantId(tenantId))
            {
                using (var context = LocalIocManager.Resolve<LittleSurveyDbContext>())
        ...
        protected T UsingDbContext<T>(int? tenantId, Func<LittleSurveyDbContext, T> func)
        {
            T result;
        ...
        protected async Task<T> UsingDbContextAsync<T>(int? tenantId, Func<LittleSurveyDbContext, Task<T>> func)
        {
            T result;
